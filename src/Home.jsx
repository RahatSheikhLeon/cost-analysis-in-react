...
import { calculateTotalBy, getAllData } from "./utils";
...
export function Home() {
	...
	const updatedMemberInfo = memberInfo.map((info) => ({
		...
		action: (
			<>
				<NavLink to={`/member/${info.id}`}>More INFO</NavLink>
			</>
		)
	}));
	...
	const [filterMember, setFilterMember] = useState(updatedMemberInfo)
	...
	function searchMember(event) {
		const filter = updatedMemberInfo.filter(row => {
		...
		<>
			...
			{oneMember == true ? <div className="home-tabel_wrapper ">
				...
				<div className="flex border-buttom table-footer">
					...
					<div className="width-50 "><h4 className="flex align-item_center">Total TK : <span className="total-tk">{calculateTotalBy()}</span></h4></div>
				</div>
			</div>
				: null}


			{oneMember == false ? <AmemberInformation memberData={memberData} setOneMember={setOneMember} memberInfo={memberInfo} totalAmounts={calculateTotalBy()} totalMealReat={calculateTotalBy('Meal')} /> : null}
		</>
	)
}